...
const fallbackImage: string =
  "https://res.cloudinary.com/dwrrey9e3/image/upload/v1706308997/ogblog.png";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${post.title}`,
    metadataBase: new URL(`https://cojocarudavid.me/blog/${post.slug}`),
    description: post.description,
    publisher: post.author.name,
    keywords: post.tags,
    alternates: {
      canonical:
        post.canonicalLink || `https://cojocarudavid.me/blog/${post.slug}`,
    },
    openGraph: {
      images:
        urlFor(post.coverImage?.image).width(1200).height(630).url() ||
        fallbackImage,
      url: `https://cojocarudavid.me/blog/${post.slug}`,
      title: post.title,
      description: post.description,
      type: "article",
      siteName: "cojocarudavid.me",
...
export default async function Post({ params }: Props) {
  ...
  function nameToUsername(author: PostType["author"]) {
    return author.name.split(" ").join("").toLowerCase();
  }
